...
const API_URL = "https://api.jikan.moe/v3/top/anime/2/bypopularity";
...
const genre =  [
    ...
    },
    {
     ...
     hentai.push(dataa[i].rated)
     ecchi.push(dataa[i].image_url)
      //console.log(dataa[i])

      if(hentai.includes(dataa[i].rated)){
                    hentai.forEach((idx) => {
                        if(dataa[i].rated == "Rx"){
                          //alert('baka')
                            hentai.splice(idx, 1);
     ...
     // const result = hentai.filter(word => word !== "rx");
